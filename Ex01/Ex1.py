...
def isort(unsortiert):
        ...
        #erster Wert bereits in Liste übernommen
                pos = 0
                while pos < len(sortiert):
                        if i > sortiert[pos]:
                                ...
                                #Wert an Position ist größer/gleich, vorher einsetzen, dann loop beenden und mit neuem i starten
...
def ll():
        liste=[]
        if isort(liste) != ("Fehler: Liste leer") or isort(1) != ("Fehler: Eingabe muss eine Liste sein"):
...
def ug():
        ...
        if isort(listeg) != [5,8,9,11,22,45] or isort(listeug) != [5,8,9,11,11,22,45]:
